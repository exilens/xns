...
func validOwnerPoint(raw []byte) error {
	...
	if eq(p, identity) {
		return errors.New("owner key must not be identity")
	}
	if !eq(scalarMult(p, edL), identity) {
		return errors.New("owner key must be in the Ed25519 prime-order subgroup")
	}
	return nil
}
...
func decodePoint(raw []byte) (point, error) {
	...
	if y.Cmp(edP) >= 0 {
		return identity, errors.New("encoded point has non-canonical y coordinate")
	}
	...
	if sub(mul(x, x), xx).Sign() != 0 {
		return identity, errors.New("encoded point is not on the Ed25519 curve")
	}
	if x.Sign() == 0 && sign == 1 {
		return identity, errors.New("encoded point has invalid sign bit for x = 0")
	}
